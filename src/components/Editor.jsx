...
function Editor({ htmlCode, setHtmlCode, cssCode, setCssCode, jsCode, setJsCode, srcDoc, setSrcDoc }) {
    ...
    const tabs = [
        { id: 'html', label: 'HTML', icon: <HtmlIcon />, lang: html, value: htmlCode, onChange: setHtmlCode },
        { id: 'css', label: 'CSS', icon: <CssIcon />, lang: css, value: cssCode, onChange: setCssCode },
        { id: 'js', label: 'JavaScript', icon: <JsIcon />, lang: javascript, value: jsCode, onChange: setJsCode },
    ];

    const activeEditor = tabs.find(tab => tab.id === activeTab);
    ...
    useEffect(() => {
        ...
        if (!resizer || !codePanel || !previewPanel || !editorContainer) return;
        ...
        const handleMouseMove = (e) => {
            ...
            let newCodePanelWidth = e.clientX - containerRect.left;
            const minWidth = 200;
            const maxWidth = containerRect.width - minWidth;
            if (newCodePanelWidth < minWidth) newCodePanelWidth = minWidth;
            if (newCodePanelWidth > maxWidth) newCodePanelWidth = maxWidth;
            const newCodePanelWidthPercent = (newCodePanelWidth / containerRect.width) * 100;
            codePanel.style.width = `${newCodePanelWidthPercent}%`;
            previewPanel.style.width = `${100 - newCodePanelWidthPercent}%`;
        };
    ...
    return (
        <main ref={editorContainerRef} id="editor-container">

            <div ref={codePanelRef} id="code-panel">
                <div className="flex bg-gray-900 place-content-between align-middle">
                    <div id="editor-tabs" className="flex bg-gray-900">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                className={`tab-btn flex-center ${
                                    activeTab === tab.id
                                    ? 'active-tab'
                                    : 'none'
                                }`}
                    ...
                    {saved?<div className='text-teal-600 text-sm flex-center px-3'>Saved</div>:''}
                </div>

                <div className="w-full h-full overflow-auto bg-gray-800 hover:cursor-text">
                    <CodeMirror
                        value={activeEditor.value}
                        height="100%"
                        extensions={[activeEditor.lang()]}
            ...
            {/* Resize Handle */}
            <div ref={resizerRef} className="resize-handle-x cursor-ew-resize w-2 bg-gray-600 hover:bg-blue-500 transition-colors duration-200"></div>


            {/* Preview Panel */}
            <div ref={previewPanelRef} id="preview-panel" className="bg-white">
                <iframe
                    id="preview-frame"
                    srcDoc={srcDoc}
                    title="preview"
